...
if raw_image_count == 0:
    print("--- ERROR: No images to process. Check your MinIO bucket 'lakehouse/images/' ---")
    spark.stop()
    exit(1)

vector_df = image_df.withColumn("vector", extract_vector_udf(col("content"))) \
...
vector_df.select("path", "processed_at") \
    .writeTo("metastore.db.image_metadata") \
    .append()
...
def insert_to_milvus(partition):
    ...
    for row in partition:
        if row.vector is not None and len(row.vector) > 0:
            ...
            print(f"--- WARNING: Found null vector for path {row.path}. Is ResNet weight downloaded? ---")
